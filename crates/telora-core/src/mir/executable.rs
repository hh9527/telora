...
impl ExecutionClosure {
    pub fn nodes(&self) -> &[ExecutionRoot] {
        &self.nodes
    }
}
...
impl<'a> SealedExecutable<'a> {
    pub fn sealed_mir(&self) -> &SealedMir<'a> {
        &self.sealed
    }
    ...
    pub fn instances(&self) -> &BTreeSet<GenericInstanceId> {
        &self.instances
    }
...
impl<'a> SealedMir<'a> {
    ...
    pub fn seal_export(self, export: SymbolId) -> Result<SealedExecutable<'a>, Vec<Diagnostic>> {
        ...
        let symbol = mir
            .symbols
            .get(export.index())
            .ok_or_else(|| failure("execution export has no symbol"))?;
        let ResolveState::Bound(target) = symbol.resolution else {
            return Err(failure("execution export is not resolved"));
        };
        if !matches!(
            mir.symbols[target.index()].kind,
        ...
        let node = *mir.symbols[target.index()]
            .declarations
            .last()
            .ok_or_else(|| failure("execution export has no declaration"))?;
        let mut roots = vec![ExecutionRoot {
            node,
            instance: None,
        }];
        ...
        let properties: Vec<usize> = mir
            .properties
            .iter()
            .enumerate()
            .filter(|(_, property)| property.concrete)
            .map(|(index, property)| {
                roots.extend(property.providers.iter().map(|&node| ExecutionRoot {
                    node,
    ...
    pub fn seal_modules(
        ...
        for (index, symbol) in mir.symbols.iter().enumerate() {
            if matches!(
                ...
                .declarations
                .iter()
                .any(|node| mir.value_materializations[node.index()].is_some())
        ...
        for &symbol in &globals {
            if mir.symbol_generics[symbol.index()].is_empty() {
                roots.extend(
                    mir.symbols[symbol.index()]
                        .declarations
                        .iter()
                        .map(|&node| ExecutionRoot {
                            node,
                            instance: None,
                        }),
                );
        ...
        for (id, instance) in mir.generic_instances() {
            if instance.concrete && globals.contains(&instance.symbol) {
                roots.extend(
                    mir.symbols[instance.symbol.index()]
                        .declarations
                        .iter()
                        .map(|&node| ExecutionRoot {
                            node,
            ...
            .filter(|(_, property)| {
                property.concrete
                    && property
                        .providers
                        .iter()
                        .any(|node| modules.contains(&mir.hir[node.index()].module))
            })
            .map(|(index, property)| {
                roots.extend(property.providers.iter().map(|&node| ExecutionRoot {
                    node,
        ...
        let checks = mir
            ...
            .enumerate()
            .filter(|(_, check)| {
                check.concrete && modules.contains(&mir.hir[check.checker.index()].module)
            })
            .map(|(index, check)| {
                roots.push(ExecutionRoot {
                    node: check.checker,
    ...
    fn publish_execution(
        ...
        for root in closure.nodes() {
            if let Some(instance) = root.instance
                && matches!(
                    mir.symbols[mir.generic_instances[instance.index()].symbol.index()].kind,
                    SymbolKind::Declaration(
                        BindingKind::Let
                            | BindingKind::Def
                            | BindingKind::Native
            ...
            let Some(symbol) = mir.hir_symbols[root.node.index()] else {
                continue;
            };
            ...
            if matches!(
                ...
                definition.scope.is_some() && definition.scope == mir.module_scopes[module.index()]
            }) {
...
impl SealedMir<'_> {
    ...
    pub fn execution_closure(
        &self,
        roots: &[ExecutionRoot],
    ) -> Result<ExecutionClosure, Vec<Diagnostic>> {
        let mir = self.mir();
        ...
        while let Some(root) = pending.pop() {
                    ...
                    .filter(|instance| instance.concrete)
                {
                    ...
                    None => {
                        diagnostics.push(Diagnostic::error(
                            "execution requires a concrete generic instance",
                            node.location,
                        ));
                        continue;
                    }
            ...
            if mir.required_types[root.node.index()] {
                let ty = if let Some(instance) = instance {
                    instance.ty(root.node)
                } else {
                    match mir.ty_slots[root.node.index()] {
                        TypeState::Known(ty) => Some(ty),
                        _ => None,
                    }
                };
                let closed = ty.is_some_and(|ty| {
                    if closed_types.contains(&ty) {
                        return true;
                    }
                    let mut types = vec![ty];
                    let mut visited = BTreeSet::new();
                    while let Some(ty) = types.pop() {
                        if closed_types.contains(&ty) || !visited.insert(ty) {
                            continue;
                        }
                        let Some(shape) = mir.types.get(ty.index()) else {
                            return false;
                        };
                        if matches!(
                            shape.constructor,
                            TypeConstructor::Parameter(_)
                                | TypeConstructor::Bound(_)
                                | TypeConstructor::Quantified(_)
                        ) {
                            return false;
                        }
                        types.extend(shape.arguments.iter().copied());
                        if let Some(layout) =
                            mir.type_layouts.get(ty.index()).and_then(Option::as_ref)
                        {
                ...
                if !closed {
                    diagnostics.push(Diagnostic::error(
                        "executable value requires a fully determined type",
                        node.location,
                    ));
                    continue;
                }
            ...
            if mir.value_materializations[root.node.index()].is_some() {
                continue;
            }
            let reference = if let Some(instance) = instance {
                instance.reference(root.node)
            } else {
                mir.generic_references[root.node.index()].and_then(GenericReference::instance)
            }
            .or_else(|| {
                if let Some(instance) = instance {
                    instance.implementation(root.node)
                } else {
            ...
            if let Some(id) = reference {
                if let Some(selected) = mir.generic_instances.get(id.index()) {
                    ...
                    diagnostics.push(Diagnostic::error(
                        "execution reference has no generic instance",
                        node.location,
                    ));
                }
                ...
                && let ResolveState::Bound(symbol) = mir.resolve_slots[slot.index()]
            {
                ...
                if symbol.module.is_some_and(|module| {
                    ...
                    pending.extend(symbol.declarations.iter().map(|&node| ExecutionRoot {
                        node,
                        instance: None,
                    }));
                }
            ...
            if let Some(MemberSelection::TraitMember {
                ...
                ..
            }) = mir.member_selections[root.node.index()]
                && reference.is_none()
            {
                pending.extend(
                    mir.symbols[symbol.index()]
            ...
            for edge in &node.children {
                if matches!(
                    ...
                    Role::Annotation
                        ...
                        | Role::Name
                        | Role::Target
                ) || matches!(node.kind, HirKind::TypeApply) && edge.role != Role::Callee
                {
                    continue;
                }
                // A template declaration is visited through its selected instances.
                if edge.role == Role::Binding
                    && mir.hir_symbols[edge.node.index()]
                        .is_some_and(|symbol| !mir.symbol_generics[symbol.index()].is_empty())
                {
                    continue;
                }
                pending.push(ExecutionRoot {
