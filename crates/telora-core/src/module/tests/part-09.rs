    ...
    #[test]
    fn dependent_property_evidence_rejects_an_unpublished_property() {
        let directory = fixture_dir();
        let main = directory.join("main.telora");
        fs::write(
            &main,
            r#"import "std/type-property" as type_property;
                   @property('Type)
                   type Prepared = struct { marker: String };
                   type Plain = struct { value: Int };
                   def evidence: for(P, T: Property(P)) Fn(TypeOf(T), TypeOf(P)) -> P =
                       fn(target, property) { type_property.evidence(target, property) };
                   export def output = evidence(Plain, Prepared);"#,
        )
        .unwrap();
        ...
        assert!(
            error
                .message()
                .contains("Plain has no published Property(Prepared) evidence"),
            "{error}"
        );
        fs::remove_dir_all(directory).unwrap();
    }
    ...
    #[test]
    fn type_desc_fields_expose_runtime_member_descriptor_shapes() {
        let directory = fixture_dir();
        let main = directory.join("main.telora");
        fs::write(
            &main,
            r#"import "std/array" as array;
                   ...
                   type Target = struct { text: String, count: Int, ratio: Float, nested: Nested };
                   export def output = array.map(type_desc.fields(Target), fn(field) {
                       type_desc.kind(type_desc.strip_attributes(field.ty))
                   });"#,
        )
        .unwrap();
        let module = load_module(&main, BTreeMap::new(), 100_000).unwrap();
        let output = module.execute(100_000).unwrap().value().to_string();
        assert_eq!(output, "{output: ['Int, 'Ref, 'Float, 'String]}");
        fs::remove_dir_all(directory).unwrap();
    }
