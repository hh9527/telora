...
impl Solver<'_> {
        ...
        for task in &self.tasks {
            let (node, operand, message) = match task {
                Task::Numeric { node, operand } => (
                    *node,
                    *operand,
                    "cannot infer numeric operand type (Int or Float required)".to_string(),
                ),
                Task::Not { node, operand } => (
                    *node,
                    *operand,
                    "cannot infer ! operand type (Int or Bool required)".to_string(),
                ),
                Task::Ordered { node, operand } => (
                    *node,
                    *operand,
                    "cannot infer ordered comparison operand type (Int, Float, or String required)"
                        .to_string(),
                ),
                Task::Member {
                    node,
                    ...
                    *node,
                    *receiver,
                    format!("cannot infer receiver type for field {name:?}"),
                ),
                _ => continue,
            };
            if self.mir.ty_slots[self.root(operand).index()] != TypeState::Unknown {
                continue;
            }
        ...
        match reference {
            Reference::Slot(slot) => match self.mir.ty_slots[self.root(slot).index()] {
                TypeState::Structure(id) => {
                    ...
                    self.render_shape(
                        &term.constructor,
                        term.arguments.iter().copied().map(Reference::Slot),
                        depth,
                        budget,
                    )
                }
            ...
            Reference::Known(id) => {
                ...
                self.render_shape(
                    &ty.constructor,
                    ty.arguments.iter().copied().map(Reference::Known),
                    depth,
                    budget,
                )
            }
    ...
    fn render_shape(
        ...
        children: impl Iterator<Item = Reference>,
        depth: usize,
        budget: &mut usize,
    ) -> String {
        let mut args = vec![];
        ...
        match constructor {
            TypeConstructor::Tuple | TypeConstructor::TupleLiteral => format!(
                "({}{})",
                args.join(", "),
                if args.len() == 1 { "," } else { "" }
            ),
            ...
            TypeConstructor::Meta | TypeConstructor::TypeOf => {
