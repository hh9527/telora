...
impl Solver<'_> {
    pub(in crate::type_resolve) fn request_evidence(
        &mut self,
        subject: TypeId,
        bound: TypeId,
    ) -> usize {
    ...
    pub(in crate::type_resolve) fn solve_pending_evidence(
        ...
        while self.evidence_cursor < self.mir.evidence.len() {
            if !self.check_type_expansion(origin) {
                for node in &mut self.mir.evidence[start..] {
                    if node.state == BoundState::Pending {
                        node.state = BoundState::Unresolved;
                    }
                }
                return false;
            }
            ...
            let (subject, bound) = (
                self.mir.evidence[index].subject,
                self.mir.evidence[index].bound,
            );
            ...
            self.mir.evidence[index].arguments =
                substitutions.iter().map(|(&p, &t)| (p, t)).collect();
        ...
        for index in start..self.mir.evidence.len() {
            ...
            for &dependency in &node.dependencies {
                if !self.mir.evidence[dependency].state.is_proven() {
                    ...
                    if dependency >= start {
                        dependents[dependency - start].push(index);
                    }
                }
            }
            if waiting[index - start] == 0 {
                ready.push_back(index);
            }
        }
        while let Some(index) = ready.pop_front() {
            let Some(implementation) = self.mir.evidence[index].implementation else {
                continue;
            };
