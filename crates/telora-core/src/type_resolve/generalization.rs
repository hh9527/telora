...
impl Solver<'_> {
    ...
    pub(super) fn prepare_generalization(&mut self) {
        ...
        for index in 0..count {
            ...
            if !matches!(
                ...
                || !self.mir.symbol_generics[index].is_empty()
            {
                continue;
            }
            let declaration = symbol.declarations[0];
            if self.child(declaration, Role::Annotation).is_some() {
                continue;
            }
            ...
            let imported = matches!(
                self.mir.hir[declaration.index()].kind,
                HirKind::Binding {
                    imported: Some(_),
                    ..
                }
            ...
            if !matches!(self.mir.hir[value.index()].kind, HirKind::Closure)
                && !(imported && self.non_expansive(value))
            {
                continue;
            }
            let mut nodes = vec![];
            ...
            self.generalizations[index] = Some(Candidate {
                nodes,
                dependencies: vec![],
                captures: vec![],
            });
        ...
        for index in 0..count {
            ...
            let nodes = if let Some(candidate) = &self.generalizations[index] {
                ...
                let mut pending = symbol
                    .declarations
                    .iter()
                    .filter_map(|&node| self.child(node, Role::Value))
                    .collect::<Vec<_>>();
                let mut nodes = vec![];
                while let Some(node) = pending.pop() {
                    nodes.push(node);
            ...
            for &node in nodes {
                ...
                if let Some(symbol) = self.mir.hir_symbols[node.index()]
                    && symbol.index() != index
                    && self.generalizations[symbol.index()].is_some()
                {
                    ...
                    && let ResolveState::Bound(symbol) = self.mir.resolve_slots[slot.index()]
                {
                    ...
                    if !self.mir.symbols[symbol.index()]
                        .declarations
                        .iter()
                        .any(|node| owned.contains(node))
                    {
        ...
        for index in 0..count {
            if self.generalizations[index].is_none() {
                continue;
            }
    ...
    pub(super) fn generalize_ready(&mut self) -> bool {
        ...
        for task in &self.tasks {
            match task {
                Task::Numeric { operand, .. }
                | Task::Not { operand, .. }
                | Task::Ordered { operand, .. } => {
                    constrained.extend(self.unknown_leaves(*operand))
                }
                Task::Member { receiver, .. }
                | Task::Projection { receiver, .. }
                | Task::FieldProjection { receiver, .. } => {
                    constrained.extend(self.unknown_leaves(*receiver))
                }
                _ => {}
            }
            let slots = match task {
                ...
                | Task::Ordered { node, operand } => vec![node.ty(), *operand],
                Task::Member { node, receiver, .. }
                | Task::Projection { node, receiver, .. }
                ...
                Task::ConstructorPattern {
                    ...
                    let mut slots = vec![node.ty(), *constructor];
                ...
                | Task::Join { node, .. }
                | Task::Block { node, .. }
                | Task::Tuple { node, .. } => vec![node.ty()],
            };
            for slot in slots {
                blocked.extend(self.unknown_leaves(slot));
            }
        }
                ...
                candidate
                    ...
                    .filter(|candidate| {
                        candidate
                            .dependencies
                            .iter()
                            .all(|symbol| self.generalizations[symbol.index()].is_none())
                    })
                    .map(|_| index)
            })
            .collect::<Vec<_>>();
        ...
        for index in ready {
            ...
            if !self
                .term(self.mir.symbol_types[index])
                .is_some_and(|term| match term.constructor {
                    TypeConstructor::Function
                    | TypeConstructor::Option
                    | TypeConstructor::Result
                    | TypeConstructor::FoldControl => true,
                    ...
                    TypeConstructor::Nominal(symbol) => {
                        self.nominal_index[symbol.index()].is_some_and(|definition| {
                            self.mir.type_definitions[definition].operation == TypeOperation::Enum
            ...
            let mut ordered = self.mir.type_instances[value.index()]
                .iter()
                .flat_map(|(_, slot)| self.unknown_leaves(*slot))
                .collect::<Vec<_>>();
            if ordered.is_empty()
                && matches!(
            ...
            if leaves
                .iter()
                .any(|slot| constrained.contains(slot) && !captured.contains(slot))
            {
                continue;
            }
            ...
            for slot in leaves {
                ...
                self.mir.symbols.push(Symbol {
                    native_type: None,
                    module: self.mir.symbols[index].module,
                    name: if ordinal < 26 {
                    ...
                    declarations: vec![],
                    scope: None,
                    resolution: ResolveState::Bound(parameter),
                });
