...
fn contract_checks_cover_value_shapes_and_unresolved_aliases() {
    ...
    for (fixture, names, unresolved) in [
        ...
        let source =
            std::fs::read_to_string(directory.join(fixture).join("testee.telora")).unwrap();
        ...
        for name in names {
            let contract = mir
                ...
                .iter()
                .find(|contract| mir.symbols[contract.symbol.index()].name == *name)
                .unwrap();
            assert!(
                if unresolved {
                    contract.state == DeclarationContractState::Unresolved
                } else {
                    contract.state == DeclarationContractState::Missing
                },
                "{name}: {contract:?}"
            );
        }
...
fn explicit_error_paths_cannot_poison_closed_contracts() {
    let source = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/contract-error-boundaries.telora"),
    )
    .unwrap();
    let mut mir = graph(&[("@src/main", &source)]);
    resolve(&mut mir);
    for name in [
        ...
        assert!(
            matches!(symbol_type(&mir, name), TypeState::Known(_)),
            "{name}: {}",
            mir.dump()
        );
    }
...
fn materialized_template_value_is_not_generalized_at_each_use() {
    ...
    assert!(
        ...
        "a function value cannot select different type arguments at each call: {}",
        mir.dump()
    );
    let valid = mir
        .symbols
        .iter()
        .find(|symbol| symbol.name == "valid")
        .unwrap();
    assert!(
        valid
            .declarations
            .iter()
            .all(|declaration| mir.type_conflicts_in(*declaration).is_empty()),
        "{:?}",
        mir.diagnostics
    );
    assert!(mir.seal().is_err());
...
fn template_references_instantiate_independently_and_share_only_closed_instances() {
    let source = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/template-materialization.telora"),
    )
    .unwrap();
    let library = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/template-library.telora"),
    )
    .unwrap();
    let mut mir = graph(&[("@src/main", &source), ("@src/library", &library)]);
    resolve(&mut mir);
    ...
    assert_eq!(
        instances.len(),
        2,
        "the two String uses can share code; the Int use cannot"
    );
    let arguments = instances
        ...
        .map(|id| {
            ...
            mir.types[instance.arguments[0].1.index()]
                .constructor
                .clone()
        })
        .collect::<BTreeSet<_>>();
    assert_eq!(
        arguments,
        BTreeSet::from([TypeConstructor::Int, TypeConstructor::String])
    );
}
...
fn explicit_export_obligations_survive_inferred_types_and_diagnostic_removal() {
    ...
    let source = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/export-contracts.telora"),
    )
    .unwrap();
    let mut mir = graph(&[("@src/main", &source)]);
    resolve(&mut mir);
    ...
    assert!(matches!(
        state("complete"),
        DeclarationContractState::Complete(_)
    ));
    assert!(matches!(
        state("identity"),
        DeclarationContractState::Complete(_)
    ...
    for name in ["missing", "alias", "private_inferred"] {
        ...
        let contract = mir
            ...
            .unwrap();
        assert!(
            !mir.declaration_contract_ready[contract.symbol.index()],
    ...
    let TypeState::Known(ty) = symbol_type(&mir, "private_inferred") else {
        panic!("{}", mir.dump());
    };
...
fn failed_generic_use_and_body_keep_other_inference_results() {
    let source = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../tests/language/src/check/diag-contract-local/testee.telora"),
    )
    .unwrap();
    let mut mir = graph(&[("@src/main", &source)]);
    resolve(&mut mir);
    for (name, expected) in [
        ("good", TypeConstructor::Int),
        ("input", TypeConstructor::String),
    ...
    let TypeState::Known(signature) = symbol_type(&mir, "broken_body") else {
        panic!("{}", mir.dump());
    };
    assert_eq!(
        mir.types[signature.index()].constructor,
        TypeConstructor::Function
    );
    assert!(
        mir.types[signature.index()]
            .arguments
            .iter()
            .all(|ty| mir.types[ty.index()].constructor == TypeConstructor::Int)
    );
    ...
    for name in ["same", "broken_body"] {
        ...
        assert!(
            mir.type_conflicts
                .iter()
                .any(|failure| failure.contracts.contains(&annotation)),
            "{name}: {:?}",
            mir.type_conflicts
        );
    }
...
fn rejected_uses_preserve_shared_contracts_and_independent_diagnostics() {
    ...
    for (root, reverse) in [("testee", false), ("reversed", true)] {
        ...
        assert!(
            mir.type_conflicts
                .iter()
                .all(|failure| !failure.contracts.is_empty())
        );
        assert!(mir.seal().is_err());
