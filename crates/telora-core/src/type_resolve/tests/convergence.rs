...
fn finite_instance_cycles_close_and_growth_does_not_cascade() {
    for case in ["diag-instance-growth", "diag-instance-mutual-growth", "diag-instance-trait-growth"] {
        ...
        assert!(mir.generic_instances.len() < 10000, "growth stopped at the type depth guard");
...
fn concrete_instance_obligations_share_one_closed_evidence_graph() {
    ...
    let entry = std::fs::read_to_string(root.join("fixtures/type-expansion-limits/evidence.telora")).unwrap();
    let provider = std::fs::read_to_string(root.join("language/src/test/instance-evidence/provider.telora")).unwrap();
    let mut mir = graph(&[("@src/main", &entry), ("@src/provider", &provider)]);
    resolve(&mut mir);
    mir.seal().unwrap_or_else(|d| panic!("{d:?}"));
    let keys = mir.evidence.iter().map(|node| (node.subject, node.bound)).collect::<BTreeSet<_>>();
    assert_eq!(keys.len(), mir.evidence.len(), "one stable node per obligation");
    assert!(mir.evidence.iter().all(|node| node.state.is_proven()));
    assert!(mir.evidence.iter().any(|node| !node.dependencies.is_empty()));
    for instance in mir.generic_instances.iter().filter(|instance| instance.concrete) {
        for &(node, implementation) in &instance.implementations {
            assert!(matches!(mir.member_selections[node.index()], Some(MemberSelection::TraitMember { .. })));
            assert!(mir.generic_instances[implementation.index()].concrete);
        }
    }
    assert!(mir.generic_instances.iter().filter(|instance| instance.concrete)
