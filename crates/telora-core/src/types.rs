...
pub(crate) fn scheme_assignable(scheme: &TypeScheme, expected: &TypeDescriptor) -> bool {
    fn collect(
        actual: &TypeDescriptor,
        expected: &TypeDescriptor,
        replacements: &mut HashMap<TypeParameterId, TypeDescriptor>,
    ) -> bool {
        if let TypeDescriptor::Bound(parameter) = actual {
            if let Some(existing) = replacements.get(parameter) {
                return assignable(existing, expected) && assignable(expected, existing);
            }
            replacements.insert(*parameter, expected.clone());
            return true;
        }
        match (actual, expected) {
            (TypeDescriptor::Array(a), TypeDescriptor::Array(e))
            | (TypeDescriptor::Dict(a), TypeDescriptor::Dict(e))
            | (TypeDescriptor::TypeOf(a), TypeDescriptor::TypeOf(e)) => collect(a, e, replacements),
            (
                TypeDescriptor::Tagged {
                    tag: at,
                    payload: a,
            ...
            ) => at == et && collect(a, e, replacements),
            (TypeDescriptor::Tuple(a), TypeDescriptor::Tuple(e)) => {
                a.len() == e.len() && a.iter().zip(e).all(|(a, e)| collect(a, e, replacements))
            }
            (TypeDescriptor::Struct(a), TypeDescriptor::Struct(e)) => {
                a.len() == e.len()
                    && e.iter()
                        .all(|(name, e)| a.get(name).is_some_and(|a| collect(a, e, replacements)))
            }
            (TypeDescriptor::Enum(a), TypeDescriptor::Enum(e)) => {
                a.len() == e.len()
                    && e.iter().all(|(name, e)| {
                        a.get(name).is_some_and(|a| match (a, e) {
                            (None, None) => true,
                            (Some(a), Some(e)) => collect(a, e, replacements),
                            _ => false,
                        })
                    })
            }
            (
                ...
                ap.len() == ep.len()
                    && ap.iter().zip(ep).all(|(a, e)| collect(a, e, replacements))
                    && collect(ar, er, replacements)
            }
            _ => true,
        }
    }
    ...
    let mut replacements = HashMap::new();
    if !collect(&scheme.body, expected, &mut replacements) {
        return false;
    }
    let instantiated = substitute_bound_parameters(&scheme.body, &replacements);
    !contains_bound_parameters(&instantiated) && assignable(&instantiated, expected)
}

fn contains_bound_parameters(descriptor: &TypeDescriptor) -> bool {
    match descriptor {
        TypeDescriptor::Bound(_) => true,
        TypeDescriptor::Array(item) | TypeDescriptor::Dict(item) | TypeDescriptor::TypeOf(item) => {
            contains_bound_parameters(item)
        }
        TypeDescriptor::Tagged { payload, .. } => contains_bound_parameters(payload),
        TypeDescriptor::Tuple(items) | TypeDescriptor::Union(items) => {
            items.iter().any(contains_bound_parameters)
        }
        TypeDescriptor::Struct(fields) => fields.values().any(contains_bound_parameters),
        TypeDescriptor::Enum(variants) => variants
            .values()
            .filter_map(Option::as_deref)
            .any(contains_bound_parameters),
        TypeDescriptor::Function { parameters, result } => {
            parameters.iter().any(contains_bound_parameters) || contains_bound_parameters(result)
        }
        _ => false,
    }
}
