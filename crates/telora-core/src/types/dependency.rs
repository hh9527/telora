...
fn intrinsic_property_decorator(decorator: &crate::ast::Decorator) -> bool {
    !decorator.value.configured
        && matches!(
...
fn decorator_call(decorator: &crate::ast::Decorator, target: &Binding) -> Expr {
    let provider = if decorator.value.configured {
        located(
            ExprKind::Call {
    ...
    let target = located(
        ExprKind::Variable(target.value.name.clone()),
        target.value.name.location,
    );
    located(
        ExprKind::Call {
            callee: Box::new(provider),
...
fn decorator_property_descriptor(
    ...
    let TypeDescriptor::Function { parameters, result } = provider else {
        return Err(FrontendError::from_diagnostic(
            sources,
            Diagnostic::error(
                "decorator must provide Fn(TypeDesc) -> Property",
                decorator.location,
            ),
        ));
    };
    if parameters.len() != 1
        || !matches!(parameters[0], TypeDescriptor::Type | TypeDescriptor::Any)
    {
        return Err(FrontendError::from_diagnostic(
            sources,
            Diagnostic::error(
                "decorator provider must accept exactly one TypeDesc",
                decorator.location,
            ),
        ));
    }
...
pub(crate) fn analyze_program_with_bindings_observed(
    ...
    let local_property_types = program
        .value
        .body
        .value
        .bindings
        ...
        .filter(|binding| {
            binding.value.kind == BindingKind::Type
                ...
                && binding
                    ...
                    .decorators
                    .iter()
                    .any(intrinsic_property_decorator)
        })
        .map(|binding| {
            let value = tool_values[&binding.value.name.value];
            evaluator.declared_type_id(value)
        })
    ...
    for binding in program
        ...
        .bindings
        .iter()
        .filter(|binding| !binding.value.decorators.is_empty())
    {
        if binding.value.kind != BindingKind::Type
            || binding.value.declared_initializer.is_none()
            || !binding.value.type_parameters.is_empty()
        ...
        let mut establish_native_decorator = None;
        let mut properties = Vec::new();
        let mut property_origins = HashMap::new();
        for decorator in &binding.value.decorators {
            let property_type;
            let property;
            if intrinsic_property_decorator(decorator) {
                let bootstrap = binding.value.name.value == "NativeDecorator"
                    && evaluator.native_decorator_type().is_none();
                property_type = if bootstrap {
                    establish_native_decorator = Some(target_type);
                    target_type
                } else {
                    evaluator.native_decorator_type().ok_or_else(|| {
                        FrontendError::from_diagnostic(
                            sources,
                            Diagnostic::error(
                                "@property requires the NativeDecorator bootstrap",
                                decorator.location,
                            ),
                        )
                    })?
                };
                property = evaluator.property_marker(property_type);
            } else {
                if matches!(
                    &decorator.value.callee.value,
                    ExprKind::Variable(name) if name.value == "property"
                    ...
                    return Err(FrontendError::from_diagnostic(
                        sources,
                        Diagnostic::error("@property does not accept arguments", decorator.location),
                    ));
                }
                let marker_type = evaluator.native_decorator_type().ok_or_else(|| {
                    FrontendError::from_diagnostic(
                        sources,
                        Diagnostic::error(
                            "decorator evaluation requires the NativeDecorator bootstrap",
                            decorator.location,
                        ),
                    )
                })?;
                let property_descriptor =
                    decorator_property_descriptor(decorator, &static_environment, sources)?;
                ...
                property_type = evaluator.canonical_type_id(&property_descriptor)?;
                if !local_property_types.contains(&property_type)
                    && !evaluator.has_type_property(property_type, marker_type)
                {
                    return Err(FrontendError::from_diagnostic(
                        sources,
                        Diagnostic::error(
                ...
                let call = decorator_call(decorator, binding);
                let mut decorator_descriptors = HashMap::new();
                let inferred = infer_expr_recorded(
                    &call,
                    &static_environment,
                    &mut decorator_descriptors,
                );
                ...
                property = evaluate_typed_tool_expression_silent(
                    source_name,
                    &call,
                    &tool_values,
            ...
            if let Some(previous) = property_origins.insert(property_type, decorator.location) {
                return Err(FrontendError::from_diagnostic(
                    sources,
                    Diagnostic::error(
                    ...
                    .with_secondary("first property is declared here", previous),
                ));
            }
            properties.push((target_type, property_type, property));
        }
        evaluator
            .publish_type_properties(establish_native_decorator, &properties)
            .map_err(|error| {
                FrontendError::from_diagnostic(
                    sources,
                    Diagnostic::error(error.to_string(), binding.location),
                )
            })?;
    }
