...
use crate::syntax::nesting::MAX_NESTING;
...
impl<'a> YamlLowerer<'a> {
    ...
    fn parse_block(&mut self, indent: usize) -> Result<DataNodeId, Diagnostic> {
        if self.depth == MAX_NESTING {
            return Err(self.line_error(
                self.position,
                format!("data syntax nesting exceeds parser limit ({MAX_NESTING})"),
            ));
        }
        self.depth += 1;
    ...
    fn parse_block_inner(&mut self, indent: usize) -> Result<DataNodeId, Diagnostic> {
        self.skip_trivia();
        let line = self
            .lines
            .get(self.position)
    ...
    fn parse_inline_value(
        ...
        self.depth += 1;
        let result = self.parse_inline_value_inner(line, text, offset);
        self.depth -= 1;
        result
    }

    fn parse_inline_value_inner(
        &mut self,
        line: usize,
        text: String,
        offset: usize,
