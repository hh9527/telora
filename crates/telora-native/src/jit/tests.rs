...
fn native_flat_map_admits_expansion_before_copying_output_values() {
    ...
    for limited in [false, true] {
        ...
        if limited {
            let limit = rt.requested_allocation_bytes() + 10_000;
            rt = rt.with_allocation_limit(limit);
        }
        let mut context = CallContext::with_runtime(rt).with_fuel(1000);
        let result = compiled.call(&mut context, &[small]).unwrap();
        assert_eq!(context.runtime().unwrap().array_len(&result).unwrap(), 1);
        let small_fuel = 1000 - context.remaining_fuel().unwrap();
        context = context.with_fuel(1000);
        let result = compiled.call(&mut context, &[large]);
        if limited {
            ...
            assert_eq!(context.runtime().unwrap().array_len(&result).unwrap(), 10_000);
            // The first call can initialize a shared export. Enlarging the
            // returned array must not add a per-element execution charge.
            assert!(1000 - context.remaining_fuel().unwrap() <= small_fuel);
        }
        assert_eq!(context.call_depth(), 0);
    }
}
