...
impl Runtime {
    ...
    pub fn dict(&mut self, ty: TypeId, loc: Location, pairs: &[(Value, Value)]) -> Result<Value> {
        ...
        let keys_id = self.push_words(Table::Arrays, keys)?;
        let values_id = self.push_words(Table::Arrays, values)?;
        self.pack(
            ty,
            loc,
            &[
    ...
    pub(super) fn dict_parts<'a>(
        ...
        let keys = self.object_words(Table::Arrays, value.words[2] as u32)?;
        let values = self.object_words(Table::Arrays, value.words[3] as u32)?;
