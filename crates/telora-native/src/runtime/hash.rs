...
impl Runtime {
    pub(super) fn hash_state(&self, value: &Value) -> Result<&Sha256> {
    ...
    pub(super) fn hash(&mut self, ty: TypeId, inputs: &[Value], loc: Location, operation: usize) -> Result<Value> {
        if operation == 0 {
            let digest = Sha256::digest(self.text(inputs[0].as_ref())?.as_str().as_bytes());
            let mut output = String::with_capacity(64);
            const HEX: &[u8] = b"0123456789abcdef";
            for byte in digest { output.push(HEX[(byte >> 4) as usize] as char); output.push(HEX[(byte & 15) as usize] as char); }
        ...
        match operation {
            ...
            5 => return self.bytes(ty, loc, &state.finalize()),
