...
export function injectBundle(module, manifest, rt) {
  ...
  const origin = loc => {
    if (!Array.isArray(loc) || loc.length !== 5 || loc.some(n => !Number.isInteger(n) || n < 0 || n > 0xffffffff)
        || (loc[1] > loc[3] || loc[1] === loc[3] && loc[2] > loc[4]) || !manifest.sources.some(source => source.id === location(loc).source)) throw Error('无效数据来源');
  };
