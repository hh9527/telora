...
fn native_check_obeys_phase_boundaries_and_preserves_failure_location() {
    ...
    for (source, message, line) in [
        (include_str!("../../../telora-native/tests/fixtures/function-before-initialization.telora"), "before its declaration", 3),
        (include_str!("../../../telora-native/tests/fixtures/function-alias-before-initialization.telora"), "uninitialized function", 2),
    ] {
    fs::write(cwd.join("src/main.telora"), source).unwrap();
    ...
    assert!(errors[0]["message"].as_str().unwrap().contains(message));
