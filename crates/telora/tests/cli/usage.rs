...
fn workspace_runtime_defaults_and_explicit_cli_overrides() {
    let cwd = fixture();
    fs::copy(
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../telora-wasm/tests/fixtures/format.telora"),
    ...
    for (runtime, flags, fuel, memory) in [
        ...
        let output = telora(&cwd)
            .args(["eval", "@src/main:answer", "--report-usage"])
            .args(flags)
            .output()
            .unwrap();
        assert!(
...
fn workspace_compiler_options_apply_without_cli_switches() {
    let cwd = fixture();
    fs::copy(
        Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/compiler-options.telora"),
        cwd.join("src/main.telora"),
    )
    .unwrap();
    for (compiler, expected) in [
        (
            serde_json::json!({"maxTupleItems":16}),
            Some("compiler.maxTupleItems = 16"),
        ),
        (
            serde_json::json!({"maxTypeArguments":16}),
            Some("compiler.maxTypeArguments = 16"),
        ),
        (
            serde_json::json!({"maxTypeDepth":1}),
            Some("compiler.maxTypeDepth = 1"),
        ),
        (
            serde_json::json!({"maxTupleItems":32,"maxTypeArguments":32}),
            None,
        ),
    ] {
    ...
    let output = telora(&cwd)
        .args(["check", "--lib", "--max-type-depth", "500"])
        .output()
        .unwrap();
    ...
    let output = telora(&cwd)
        .args(["check", "std/prelude", "--only-types"])
        .output()
        .unwrap();
    assert!(!output.status.success());
...
fn execution_usage_reports_fuel_and_page_aligned_mib_limits() {
    ...
    assert!(consumed > 0);
    assert_eq!(
        consumed + usage["fuel"]["remaining"].as_u64().unwrap(),
        200_000_000
    );
    ...
    let static_only = telora(&cwd)
        .args(["check", "@src/main", "--only-types", "--report-usage"])
        .output()
        .unwrap();
