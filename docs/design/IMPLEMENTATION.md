...
#190 的深度保护试验在上述规则之外增加 `expansion_limits`：初始限制为归一化类型
结构深度 256（叶节点为 1）、Tuple 单元数 1024、其他类型节点参数数 4096。
这些是编译期结构限制，不是实例总量限制，也不使用 Wasm fuel。名义类型的成员布局
不作为该名义类型的子参数递归计算；布局自身的结构节点仍受检查。每个已解析类型
在子类型之后进入数组，深度按 TypeId 增量记录，每个节点只计算一次。
...
后续展开，而不是已经实现按失败依赖分量继续处理的 best-effort 调度。

保守收敛检查暂时保留。试验中撤掉它以后，普通/互递归函数的增长能由深度保护终止，
但 `diag-instance-trait-growth` 的新具体调用缺少新增的 implementation evidence，
会提前产生 `generic trait member has no closed implementation evidence`。
后续需让具体实例产生的 trait 义务进入统一、去重的 evidence 求解队列，再解除
“无法证明有限展开”对程序的提前拒绝。本阶段尚未完成所有分析路径的停机性证明。
