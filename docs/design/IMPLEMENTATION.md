...
带约束的类型族应用或未解析名称等形式仍走明确的旧求值路径，类型声明本体、约束和 property 阶段
也尚未全部静态化，不能据此声称整个推导过程已零执行。

类型图接收到名义引用占位后，后来的完整定义可填充原类型行，已有引用保留 ID。
具体名义类型的递归分量现在先登记全部身份，再静态展开各类型体，并直接以节点 ID
填充名义类型行。支持的自递归／互递归声明无需 VM fuel；旧消费者仍需要预留、封闭
运行时类型引用并解码元数据。递归泛型及未支持的类型表达式仍保留旧求值路径。
...
以保留开放函数的 arity 和既有错误行为。运行时元数据构建与 owner 泛型替换仍有
descriptor 适配，后续按 RFC 0280 迁移；当前并非所有工具消费者都已直接读图。
