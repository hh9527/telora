...
词法闭包开始支持按稳定 SymbolId 排序的捕获计划；参数和函数内部声明不计入外部捕获。运行时环境独立存入 Vec 槽位，其缓冲区包含完整捕获值；环境 0 表示无捕获，其余编码为环境表 HeapRef + 1。初始化发布会复制嵌套闭包与捕获对象，并保持别名；FunctionId 保留为代码计划身份，不保存机器码地址。目前验证带捕获的直接调用，间接调用和完整代码计划生命周期绑定仍待完成。
