...
基础 codec 解码已接入封闭目标类型，支持标量、Option、Array、Tuple/Unit、结构记录和 Dict；字段缺失/多余或值不匹配返回携带来源的 `Err(BlameError)`，仅在用户调用 `raise!` 时转成执行诊断。字符串等叶子复用原对象描述符，不经过旧 VM 或 host Value 树。名义类型解码及其 property/构造检查尚未接入，明确拒绝，不生成未经检查的值。

仍未完成完整语法/native API 覆盖（包括 codec 属性编码、名义类型解码）及配额语义，因此不据此宣称整体路线落地。
