...
2. codegen 和链接只保留从 executable roots 可达的函数与 helper；
3. 初始化完成时，从持久函数值的 FuncId 扩展其 demand/property 根；
4. 初始化调度区分最终发布值和仅用于构造它们的中间值；
5. snapshot 只保存数据可达根与函数体依赖要求的状态槽。
...
函数体依赖不能单独替代 GC；GC 也不能凭堆数据推断稳定 ID 指向的顶层值。
...
6. 单独评审 demand 根裁剪与初始化 collection 接入；以 #218 等真实模型记录 live set、
   初始化峰值、制品大小和时间，不把累计 arena len 当作 live-set 指标。
7. 实现稳定后更新 RFC 0302 的后继说明及当前设计文档；历史 RFC 不回写设计正文。
...
- 接入初始化根后，专门 fixture 证明函数体按顶层 ID 访问的值不会被回收，纯局部
  中间值可以回收，`check` 多根诊断仍完整。
- #218 的后续测量同时报告初始化累计分配、collection 后 live set、Vec capacity 和
  Wasm 线性内存，能够区分语言活跃数据与分配器余量。
