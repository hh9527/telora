...
while IFS= read -r file; do
    ...
    # Generated grammar state machines are not handwritten module structure.
    if head -n 1 "$file" | grep -q '^// @generated by cargo run -p telora-parser-gen; do not edit\.$'; then
        continue
    fi
    lines=$(wc -l < "$file")
